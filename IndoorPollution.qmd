...
model_region_reg<-
  recipe(percent_IAP~region*year,
         data = model_data)

step_interact(terms= c("region","year"))
  #generate a recipe -- what variables do we have in y = mx+b +b(new variable will adjust slope beyond the baseline)
...
model_region_recipe <- recipe("percent_IAP", data = model_data) %>%
  
  step_interact(terms = c("region", "year")) # define a step for interaction between region and year
...
This dataset visualization shows a clear trend where GDP and deaths are clearly related:
:::
